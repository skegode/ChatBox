...
function SearchableSelect(props: {
  ...
  const { options, value, onChange, placeholder = 'Select...', disabled, required } = props;
      ...
      {open && (
        ...
        >
          <div style={{ padding: 8 }}>
            ...
            >
              {filtered.length === 0 && (
                <li style={{ padding: '8px 10px', color: '#666' }}>No results</li>
              )}
              {filtered.map((opt, i) => {
                  ...
                  <li key={opt.id}>
                    <button
                      ...
                      style={{
                        ...
                        textAlign: 'left',
                        padding: '8px 10px',
                        background: isHighlighted ? 'rgba(0,123,255,0.08)' : 'transparent',
                        border: 'none',
                        cursor: 'pointer',
                      }}
                    >
                      <span style={{ flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{opt.name}</span>
                      {isSelected && <span style={{ color: '#0d6efd', marginLeft: 8 }}>✓</span>}
                    </button>
                  </li>
                );
              })}
      ...
      <form className="mx-auto" style={{ maxWidth: 600, width: '100%', background: '#fff', borderRadius: 8, boxShadow: '0 1px 3px rgba(60,64,67,.1), 0 4px 8px rgba(60,64,67,.08)', padding: 28 }} onSubmit={handleSubmit}>
        ...
        <div className="mb-3">
          ...
          <SearchableSelect
            ...
            placeholder={loadingOptions ? 'Loading phone types…' : 'Select phone model'}
